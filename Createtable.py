...
class CreateTable:
    ...
    def instructorOfSubject(self, event):
        ...
        if subject_type == "Lecture":
            # Check for Dr (Lecturer) availability
        ...
        if instructorsList:
            ...
            messagebox.showinfo("Info", "No available instructors for the selected subject.")
    ...
    def load_schedule(self):
        try:
            ...
            if len(rows) == 0:
                print("No data found. Keeping the table structure intact.")
                return  
            ...
            for ID, level_ID, subject_ID, instructor_ID, Location_ID, Day, sections, time_start in rows:
                ...
                instructor = self.drData[instructor_ID].name if subject_type == "Lecture" else self.TAData[instructor_ID].name
        ...
        try:
            ...
            for instructor_ID, subject_ID, No_sections in rows:
                try:
                    ...
                    print(f"KeyError: instructor_ID={instructor_ID}, subject_ID={subject_ID}")
            db.close()
        except mysql.connector.Error as err:
            messagebox.showerror("Database Error", f"Error: {err}")
