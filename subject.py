...
class Subject:
    def __init__(self, master):
        self.master = master
        # Remove the line self.master.title("Subject") since 'Frame' does not have 'title'
        ...
        # Initialize levels_dic
        self.levels_dic = {}

        # Labels and Entry widgets
        self.departmentLabel = Label(self.master, text="Department:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.departmentLabel.grid(row=0, column=0)
        self.departmentVar = StringVar()
        self.departmentEntry = ttk.Combobox(self.master, textvariable=self.departmentVar)
        ...
        self.levelLabel = Label(self.master, text="Level:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.levelLabel.grid(row=1, column=0)
        self.levelVar = StringVar()
        self.levelEntry = ttk.Combobox(self.master, textvariable=self.levelVar)
        self.levelEntry.grid(row=1, column=1, padx=10, pady=10)

        self.nameLabel = Label(self.master, text="Name:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.nameLabel.grid(row=2, column=0)
        self.nameVar = StringVar()
        self.nameEntry = Entry(self.master, width=20, textvariable=self.nameVar)
        self.nameEntry.grid(row=2, column=1, padx=10, pady=10)

        self.timePerWeekLabel = Label(self.master, text="Time per week:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.timePerWeekLabel.grid(row=3, column=0)
        self.timePerWeekVar = IntVar()
        self.timePerWeekEntry = Entry(self.master, width=20, textvariable=self.timePerWeekVar)
        self.timePerWeekEntry.grid(row=3, column=1, padx=10, pady=10)

        self.sectionHourLabel = Label(self.master, text="Section hour:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.sectionHourLabel.grid(row=4, column=0)
        self.sectionHourVar = IntVar()
        self.sectionHourEntry = Entry(self.master, width=20, textvariable=self.sectionHourVar)
        self.sectionHourEntry.grid(row=4, column=1, padx=10, pady=10)

        self.lectureHourLabel = Label(self.master, text="Lecture hour:", bg='lightblue', font=('Arial', 12, 'bold'))
        self.lectureHourLabel.grid(row=5, column=0)
        self.lectureHourVar = IntVar()
        self.lectureHourEntry = Entry(self.master, width=20, textvariable=self.lectureHourVar)
        ...
        self.tree = ttk.Treeview(self.master, columns=('ID', 'level_ID', 'Name', 'Time_per_week', 'Sectionhour', 'Lecturehour'), show='headings')
    ...
    def insert_to_db(self):
        level_ID = self.levels_dic.get(int(self.levelVar.get()))
    ...
    def load_levels(self, event):
        ...
        try:
            ...
            dept_id = self.department_dict.get(self.departmentVar.get())
            dept_id = str(dept_id)
